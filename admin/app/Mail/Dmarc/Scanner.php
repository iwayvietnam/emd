...
final class Scanner
{
    ...
    public function scan(
        ...
        if (!empty($folder)) {
            ...
            foreach ($messages as $message) {
                ...
                foreach ($reports as $report) {
                    ...
                    if (!empty($report->report_metadata)) {
                        ...
                        ])->firstOr(function () use ($report, $models) {
                            $metadata = $report->report_metadata;
                            $policy = $report->policy_published;
                            $models[] = DmarcReport::create([
                                "report_id" => $metadata?->report_id ?? "",
                                "org_name" => $metadata?->org_name ?? "",
                                "org_email" => $metadata?->email ?? null,
                                "extra_contact" =>
                                    $metadata?->xtra_contact_info ?? null,
            ...
            $this->indexReports($dmarcReports);
        }

        return $models;
    }

    private function indexReports(array $reports): void
    {
